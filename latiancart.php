...
if (isset($_GET['id'])) {
  $id = $_GET['id'];
  include "latianprosescart.php";
  $query = "select id_produk_beli, quantity, nama_produk, harga from keranjang inner join produk on produk.id = keranjang.id_produk_beli where id_user = $idu and id_produk_beli = $id_produk";
  ...
  if (mysqli_num_rows($result) > 0) {
                        ...
                        <tr>
                          <th class="ps-0 py-3 border-0" scope="row">
                            <div class="d-flex align-items-center"><a class="reset-anchor d-block animsition-link"
                                href="detail.php"><img src="img/product-detail-2.jpg" alt="..." width="70" /></a>
                              <div class="ms-3"><strong class="h6"><a class="reset-anchor animsition-link"
                                    href="detail.php"><?= $data['nama_produk'] ?></a></strong></div>
                            </div>
                          </th>
                          <td class="p-3 align-middle border-0">
                            <p class="mb-0 small"><?= $data['harga'] ?></p>
                          </td>
                          <form action="latiancart.php" method="POST">
                          <td class="p-3 align-middle border-0">
                            <div class="border d-flex align-items-center justify-content-between px-3"><span
                              ...
                              <div class="quantity">
                                <button type="button"  onclick="enableInput(this)" class="dec-btn p-0"><i
                                    class="fas fa-caret-left"></i></button>
                                <input name="quantity" class="form-control form-control-sm border-0 shadow-0 p-0" type="text"
                                  value="<?= $data['quantity']?>" />
                                <button type="button" name="updateQuantity" onclick="enableInput(this)" class="inc-btn p-0"><i
                                    class="fas fa-caret-right"></i><a href="latiancart.php?id=<?= $data['id_produk_beli']?>"></button>                                                      
                              </div>
                            </div>
                          </td>
                          </form>
            ...
            <div class="bg-light px-4 py-3">
              <div class="row align-items-center text-center">
                <div class="col-md-6 mb-3 mb-md-0 text-md-start"><a class="btn btn-link p-0 text-dark btn-sm"
                    href="shop.php"><i class="fas fa-long-arrow-alt-left me-2"> </i>Continue shopping</a></div>
                <div class="col-md-6 text-md-end"><a class="btn btn-outline-dark btn-sm"
                    href="https://wa.me/62895377219504">Procceed to checkout<i
              ...
              <div class="card-body">
                ...
                <ul class="list-unstyled mb-0">
                  <li class="d-flex align-items-center justify-content-between"><strong
                      ...
                      class="text-muted small"><?= $total?></span></li>
                  ...
                  <li>
                    <form action="#">
                      <div class="input-group mb-0">
                        ...
                        <button class="btn btn-dark btn-sm w-100" type="submit"> <i class="fas fa-gift me-2"></i>Apply
                          coupon</button>
                      </div>
                    </form>
                  </li>
    ...
    <script>
      function enableInput(button) {
        ...
        const input = row.querySelector('.borrow-input');
        const availableQty = parseInt(row.querySelector('.available-qty').innerText);
        input.style.display = 'inline';
        input.addEventListener('input', () => {
          if (parseInt(input.value) > availableQty) {
            input.nextElementSibling.innerText = 'Terlalu banyak';
          } else {
